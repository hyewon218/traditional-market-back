...
$(document).ready(function() {
    ...
    loadMarkets();
    loadAllShops(currentPage, selectedCategory, searchQuery);
    ...
    // 전체 상점 목록을 가져오는 함수
    function loadAllShops(page, category, keyword) {
        let url = `/api/shops?page=${page}&size=3&sort=${sortBy}`;
        if (category) {
            url = `/api/shops/category?page=${page}&size=3&sort=${sortBy}&category=${category}`;
        }
        if(keyword) {
            url = `/api/shops/search?keyword=${keyword}&page=${page}&size=3&sort=${sortBy}`;
        }
    ...
    function renderPagination(totalPages) {
        ...
        $(".page-btn").click(function() {
            ...
            const selectedMarketNo = $('#marketNo').val();
            if (selectedMarketNo === 'all') {
                loadAllShops(newPage, selectedCategory, searchQuery);
            } else if (selectedMarketNo) {
                loadShopList(selectedMarketNo, currentPage, selectedCategory);
            }
        });

...
public class TokenProvider {
    ...
    public boolean validToken(String accessToken) {
        ...
        try {
            if (redisUtils.hasBlack(getMemberId(accessToken))) {
                ...
                log.info("블랙리스트에 저장된 value : " + redisUtils.getBlackListValue(
                    getMemberId(accessToken))); // 지우기
    ...
    public boolean validRefreshToken(String refreshToken, HttpServletRequest request,
...
//        CookieUtil.addCookie(response, REFRESH_TOKEN_COOKIE_NAME, encodeCode, cookieMaxAge);
        CookieUtil.addCookieForRefreshToken(response, REFRESH_TOKEN_COOKIE_NAME, encodeCode,
            cookieMaxAge);
    }

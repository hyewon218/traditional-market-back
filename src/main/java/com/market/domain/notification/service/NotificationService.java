...
public class NotificationService {
    ...
    public void send(NotificationType type, NotificationArgs args, Member receiver) {
        log.info("Receiver MemberNo: " + receiver.getMemberNo());
        ...
        Optional<SseEmitter> emitterOptional = emitterRepository.get(receiver.getMemberNo());

        // emitter(이벤트 발송기)의 존재 여부를 로그로 기록
        if (emitterOptional.isPresent()) {
            log.info("Emitter found for MemberNo: " + receiver.getMemberNo());
        } else {
            log.info("No emitter found for MemberNo: " + receiver.getMemberNo());
        }
        ...
        emitterOptional.ifPresentOrElse(
            sseEmitter -> {
                try {
                    ...
                    log.info(
                        "Notification sent successfully to MemberNo: " + receiver.getMemberNo());
                } catch (IOException exception) {
                    // 전송 실패 시 오류를 기록하고 처리
                    log.error("Failed to send notification. Removing emitter for MemberNo: "
                        + receiver.getMemberNo(), exception);
    ...
    public Long countNotifications(Long memberNo) {
        return notificationRepository.countByMember_MemberNoAndIsRead(memberNo, false);
    }
